...
var _ = Describe("Firedoor operator", Ordered, Serial, func() {
	...
	It("should create a recurring Breakglass resource and reach RecurringPending", func() {
		name := randomName("recurring-debug-access")
		By(fmt.Sprintf("creating breakglass - %s", name))
		bg := &accessv1alpha1.Breakglass{
			...
			Spec: accessv1alpha1.BreakglassSpec{
				...
				Subjects: []rbacv1.Subject{
					{Kind: "User", Name: "mail@matthewmcleod.co.uk", APIGroup: "rbac.authorization.k8s.io"},
				},
				...
				Schedule: accessv1alpha1.ScheduleSpec{
					Start:          metav1.Time{Time: time.Date(2024, 1, 1, 9, 0, 0, 0, time.FixedZone("EST", -5*3600))},
					Duration:       metav1.Duration{Duration: 8 * time.Hour},
					Cron:           "0 9 * * 1-5",
					Location:       "America/New_York",
					MaxActivations: int32Ptr(10),
				},
			},
		}
		...
		By("waiting for the Breakglass resource to reach RecurringPending status")
		Eventually(func() string {
			...
			return last.Type
		}, timeout, pollInterval).Should(Equal("RecurringPending"))
	})
})
